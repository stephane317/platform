...
describe('BooksReducer', () => {
  ...
  describe('SEARCH_COMPLETE & LOAD_SUCCESS', () => {
    ...
    it('should add all books in the payload when none exist', () => {
      ...
      noExistingBooks(LoadSuccessAction, fromBooks.initialState, initialState, [
        book1,
        book2,
      ]);
    });
    ...
    it('should add only new books when books already exist', () => {
      existingBooks(SearchCompleteAction, initialState, [book2, book3]);

      existingBooks(LoadSuccessAction, initialState, [book2, book3]);
    });
  });
  ...
  describe('LOAD', () => {
    ...
    it('should add a single book, if the book does not exist', () => {
      const action = new LoadAction(book1);
  ...
  describe('SELECT', () => {
    it('should set the selected book id on the state', () => {
      const action = new SelectAction(book1.id);
